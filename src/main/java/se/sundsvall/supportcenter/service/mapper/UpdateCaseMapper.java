...
public class UpdateCaseMapper {
	...
	private static List<PobPayload> toPobPayloadsWhenStatusIsPresent(String caseId, UpdateCaseRequest updateCaseRequest) {
		...
		// START REMOVE
		if (List.of(RESERVED.getValue(), PICKING.getValue(), DESPATCHED.getValue(), PARTIALLY_DESPATCHED.getValue(), DELIVERED.getValue()).stream()
			.anyMatch(ignoredStatus -> equalsIgnoreCase(ignoredStatus, updateCaseRequest.getCaseStatus()))) {
			return emptyList();
		}
		// END REMOVE
	...
	private static Note createStatusNote(NoteType noteType, String caseStatus) {
		return isNull(noteType) ? null : Note.create().withType(noteType).withText(format(NOTE_STATUS_PART, caseStatus));
	}
}
