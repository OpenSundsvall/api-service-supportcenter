...
class UpdateCaseServiceTest {
	...
	void updateCase() {
		...
		processorListSpy.stream().forEach(processor -> {
			verify(processor, times(2)).shouldProcess(request);
		});
	...
	void updateCaseForCubeStatusResolvedWillTriggerTwoPOBCalls() {
		...
		processorListSpy.stream().forEach(processor -> {
			verify(processor, times(4)).shouldProcess(request);
		});
		verify(falseProcessorMock, never()).preProcess(any(), any(), any(), any());
		verify(falseProcessorMock, never()).postProcess(any(), any(), any(), any());
	...
	void updateCaseForNetstatStatusDeliveredWillTriggerTwoPOBCalls() {
		...
		verify(pobClientMock, times(2)).updateCase(eq(pobKey), pobPayloadCaptor.capture());
		processorListSpy.stream().forEach(processor -> {
			verify(processor, atLeastOnce()).shouldProcess(request);
		});
		verify(falseProcessorMock, never()).preProcess(any(), any(), any(), any());
		verify(falseProcessorMock, never()).postProcess(any(), any(), any(), any());
		verify(trueProcessorMock).preProcess(eq(pobKey), eq(caseId), eq(request), any(PobPayload.class));
		...
		assertThat(pobPayloadValues.get(1).getMemo()).isNotNull();
		assertThat(pobPayloadValues.get(1).getMemo().get(CUSTOM_STATUS_MAP.get(caseStatus).get(1).getStatusNoteType().toValue()).getMemo()).isEqualTo("Status: '" + caseStatus + "'");
		assertThat(pobPayloadValues.get(1).getMemo().get(NoteType.PROBLEM.toValue()).getMemo()).isEqualTo(noteText);
	}
